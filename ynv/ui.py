...
class SOLLUMZ_PT_NAVMESH_POLY_ATTRS_PANEL(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        grid.prop(poly_access, "is_unk3")
        grid.prop(poly_access, "is_sheltered")
        grid.prop(poly_access, "is_unused10")
        grid.prop(poly_access, "is_unused20")
        grid.prop(poly_access, "is_switched_off_for_peds")
        grid.prop(poly_access, "is_water")

        # Flags2
        grid.prop(poly_access, "is_danger_keep_away1")
        grid.prop(poly_access, "is_danger_keep_away2")
        grid.prop(poly_access, "is_sheltered2")
        grid.prop(poly_access, "is_unk12")
        grid.prop(poly_access, "is_near_car_node")
        grid.prop(poly_access, "is_interior")
        grid.prop(poly_access, "is_isolated")
        grid.prop(poly_access, "is_zero_area_stitch")
        grid.prop(poly_access, "is_network_spawn_candidate")
        grid.prop(poly_access, "is_road")
        grid.prop(poly_access, "is_unk19")
        grid.prop(poly_access, "is_lies_along_edge")

        # Flags3
        grid.prop(poly_access, "is_unk21")
        grid.prop(poly_access, "is_unk22")
        grid.prop(poly_access, "is_unk23")
        grid.prop(poly_access, "is_muddy")
        grid.prop(poly_access, "is_sheltered3")
        grid.prop(poly_access, "is_vegetation")
        grid.prop(poly_access, "is_boardwalk")
        grid.prop(poly_access, "is_pavement")
        grid.prop(poly_access, "is_unk29")
        grid.prop(poly_access, "is_door")
        grid.prop(poly_access, "is_stairs")
        grid.prop(poly_access, "is_steep_slope")

        # Flags4
        grid.prop(poly_access, "is_unk33")
        grid.prop(poly_access, "is_shelter4")
        grid.prop(poly_access, "is_unk35")
        grid.prop(poly_access, "is_unk36")

        # Other attributes
        col = layout.column(align=True)
        col.prop(poly_access, "audio_property", slider=True)
...
class SOLLUMZ_PT_NAVMESH_EDGE_ATTRS_PANEL(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.prop(edge_access, "area_id")
        col.prop(edge_access, "poly_id")
        col.prop(edge_access, "space_around_vertex")
        col.prop(edge_access, "flags")
...
class SOLLUMZ_PT_NAVMESH_POLY_RENDER_PANEL(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        from .navmesh_material import ALL_FLAGS
...
class SOLLUMZ_PT_NAVMESH_LINK_PANEL(Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(link_props, "area_from")
        layout.prop(link_props, "area_to")
...
class SOLLUMZ_PT_NAVMESH_TOOL_PANEL(Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(wm, "sz_ui_nav_compute_edge_neighbors")
        layout.operator(nav_ops.SOLLUMZ_OT_navmesh_polys_select_similar.bl_idname)
        layout.operator(nav_ops.NAVMESH_OT_generate_from_collision.bl_idname)
