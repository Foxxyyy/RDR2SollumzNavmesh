...
def flag_enabled(data: Floaty, bit: int) -> FloatExpr:
    """Return 1.0 if the bit is set in data, else 0.0."""
    return ((truncf(data / float(bit))) % 2.0)


def flag_enabled_float(mask: FloatExpr, flag_value: int) -> FloatExpr:
    eps = 0.00001
    return ((mask / float(flag_value)) % 2.0) > (1.0 - eps)

def greater_than(a: Floaty, b: Floaty) -> FloatExpr:
    """Return 1.0 if a > b else 0.0."""
    return truncf(a - b) > 0.0


def bitwise_and(a: Floaty, mask: int) -> FloatExpr:
    """Simulate bitwise AND in shader: returns 1.0 if any masked bit is set, else 0.0."""
    return truncf(a / mask) % 2.0


def bitwise_shift_right(value: Floaty, bits: int) -> FloatExpr:
    """Shift a FloatExpr right by integer bits using division."""
